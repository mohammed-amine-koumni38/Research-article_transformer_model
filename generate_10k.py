...
"""
generate_10k.py
Générer 10 000 mots de passe depuis le point de contrôle du Transformer décodeur uniquement.

Utilisation :
  python generate_10k.py --model best_decoder2_model.pt
"""
...
parser = argparse.ArgumentParser(description="Générer 10 000 mots de passe depuis le modèle decoder2 (génération uniquement)")
parser.add_argument("--model", type=str, default="best_decoder2_model.pt")
parser.add_argument("--num", type=int, default=10000)
parser.add_argument("--output", type=str, default="10k_decoder2.txt")
...
def choose_device(device_arg):
    ...
    if arch_list and capability_sm not in arch_list:
        ...
        raise RuntimeError(
            f"CUDA device {gpu_name} ({capability_sm}) is not supported by this torch build "
            f"({', '.join(arch_list)}). Install a compatible torch build or use a supported GPU type."
        )
