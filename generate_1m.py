...
"""
generate_1m.py
Générer 1 000 000 de mots de passe depuis le point de contrôle du Transformer décodeur uniquement.

Ce script est uniquement pour la génération (sans entraînement), adapté aux points de contrôle
sauvegardés par transformer_decoder-2.py.

Utilisation :
  python generate_1m.py --model best_decoder2_model.pt
"""
...
parser = argparse.ArgumentParser(description="Générer 1M de mots de passe depuis le modèle decoder2 (génération uniquement)")
parser.add_argument("--model", type=str, default="best_decoder2_model.pt", help="Chemin vers le point de contrôle decoder2")
parser.add_argument("--num", type=int, default=1000000, help="Nombre de mots de passe à générer")
parser.add_argument("--output", type=str, default="1000k_decoder2.txt", help="Fichier de sortie")
...
parser.add_argument("--gen_batch_size", type=int, default=512, help="Taille de lot pour la génération parallèle")
parser.add_argument("--char_bias_strength", type=float, default=0.0, help="Biais de fréquence des caractères depuis les données d'entraînement (0=désactivé)")
parser.add_argument("--train", type=str, default="train.txt", help="Fichier d'entraînement utilisé pour le biais de fréquence des caractères")

parser.add_argument("--match_eval_length", action="store_true", default=True, help="Correspondre la distribution de longueur depuis le fichier d'évaluation")
parser.add_argument("--eval", type=str, default="eval.txt", help="Fichier d'évaluation utilisé uniquement pour la distribution de longueur")
parser.add_argument("--seed", type=int, default=42)
parser.add_argument("--resume", action="store_true", help="Reprendre la génération en ajoutant au fichier de sortie existant")
parser.add_argument("--flush_every", type=int, default=5000, help="Vider la sortie toutes les N mots de passe générés")
parser.add_argument("--device", choices=["cpu", "cuda"], default="cuda", help="Dispositif d'exécution (par défaut : cuda, sans repli automatique)")
...
def choose_device(device_arg):
  ...
  # Échouer rapidement si le build torch installé ne supporte pas cette architecture GPU.
  arch_list = []
...
def ensure_runtime_compatible(model, device, device_arg):
  if device.type != "cuda":
    return model, device

  try:
    with torch.no_grad():
      _ = model(torch.tensor([[START_IDX]], dtype=torch.long, device=device))
    return model, device
  except RuntimeError as exc:
    message = str(exc).lower()
    kernel_error = "no kernel image is available" in message or "invalid device function" in message

    if kernel_error:
      print(
        "Warning: CUDA kernel image incompatibility detected during runtime. "
        "Falling back to CPU for generation."
      )
...
def sample_next_index(logits, strategy, temperature, top_p):
  logits = logits.clone()
  logits[PAD_IDX] = float("-inf")
  logits = logits / max(temperature, 1e-4)
...
def generate_password_beam(model, device, max_steps, beam_width, temperature, min_length=1):
    ...
    for _ in range(max_steps):
      ...
      for tokens, score, finished in beams:
        ...
        for logp, idx in zip(top_vals.tolist(), top_idx.tolist()):
          ...
          is_finished = idx == END_IDX
          candidates.append((new_tokens, score + float(logp), is_finished))

      if all_finished:
        break

      candidates.sort(
        key=lambda x: x[1] / max(1.0, (len(x[0]) - 1) ** 0.7),
        reverse=True,
      )
...
def generate_password(model, device, strategy, temperature, top_p, beam_width, model_max_len, target_length=None, max_gen_len=20):
  max_steps = int(target_length) if target_length is not None else int(max_gen_len)
  max_steps = min(max_steps, model_max_len - 1)
  max_steps = max(1, max_steps)

  if strategy == "beam":
    return generate_password_beam(
      model,
      device,
      max_steps=max_steps,
  ...
  with torch.no_grad():
    for _ in range(max_steps):
      ...
      next_idx = sample_next_index(logits, strategy=strategy, temperature=temperature, top_p=top_p)
      if next_idx == END_IDX:
        break
      generated.append(next_idx)

  return decode_password(generated[1:])


def main():
  set_seed(args.seed)

  device, device_note = choose_device(args.device)
  print("Device:", device)
  if device_note:
    print(device_note)
  if device.type == "cuda":
  ...
  model, device = ensure_runtime_compatible(model, device, args.device)
  model.eval()

  print("Model loaded.")
  print(
    f"Config: d_model={d_model}, nhead={nhead}, num_layers={num_layers}, "
    f"ff={dim_feedforward}, dropout={dropout}, max_len={max_len}, norm_first={norm_first}, vocab_size={n_characters}"
  )

  length_values, length_probs = None, None
  if args.match_eval_length:
    ...
    if length_values is None:
      print("Eval length distribution unavailable (empty eval file). Using fixed max_gen_len.")
    else:
      print("Using eval length distribution for generation.")
  ...
  if args.resume and os.path.exists(args.output):
    ...
    if start_count >= args.num:
      print(f"Output already has {start_count:,} lines (>= requested {args.num:,}). Nothing to do.")
      print("Done: generation only.")
      return

    write_mode = "a"
    print(f"Resuming from {start_count:,}/{args.num:,} using existing file: {args.output}")

  remaining = args.num - start_count
  print(f"Generating {remaining:,} passwords ({start_count:,} already present)...")
  ...
  if args.strategy == "beam":
    # La recherche par faisceau doit rester séquentielle
    ...
    with open(args.output, write_mode, encoding="utf-8") as f:
      with tqdm(total=args.num, initial=start_count, desc="Generating (beam)") as pbar:
        for _ in range(remaining):
          target_length = None
          if length_values is not None and length_probs is not None:
            target_length = int(np.random.choice(length_values, p=length_probs))
          ...
          f.write(pw + "\n")
          generated_count += 1
          if generated_count % flush_every == 0:
            f.flush()
          pbar.update(1)
  else:
    # Génération parallèle en lots (greedy / nucleus) — beaucoup plus rapide
